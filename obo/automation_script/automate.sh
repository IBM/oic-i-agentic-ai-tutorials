...
ORCHESTRATE_MCP_TOOLKIT_NAME="mcp_toolkit"
SCOPE="mcp.read"
AUDIENCE="api://default"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ENV_OUTPUT_FILE="${SCRIPT_DIR}/../.env"
...
sed -i "" "s/^app_id:.*/app_id: ${ORCHESTRATE_CONNECTION_APP_NAME}/" \
  "${SCRIPT_DIR}/connection.yaml"

$ORCHESTRATE_CMD connections import -f "${SCRIPT_DIR}/connection.yaml"
...
$ORCHESTRATE_CMD tools import -f "${SCRIPT_DIR}/RBAC_AGENTS_TOOL/rbac_plugin.py" -k python
$ORCHESTRATE_CMD agents import -f "${SCRIPT_DIR}/RBAC_AGENTS_TOOL/hr_admin_agent.yaml"
$ORCHESTRATE_CMD agents import -f "${SCRIPT_DIR}/RBAC_AGENTS_TOOL/hr_general_agent.yaml"
$ORCHESTRATE_CMD agents import -f "${SCRIPT_DIR}/RBAC_AGENTS_TOOL/hr_main_agent.yaml"
